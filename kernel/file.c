...
int devfb_write(int user_src, uint64 src, int off, int n, void *content) {
    ...
    if (off + len > the_fb.size)
        len = the_fb.size - off;
    if (either_copyin((char *) the_fb.fb + off, user_src, src, len) == -1) /* STUDENT_TODO: replace this */
        goto out; 
    ret = len;
    __asm_flush_dcache_range((char *)the_fb.fb + off, (char *)the_fb.fb + off + len); /* STUDENT_TODO: replace this */
